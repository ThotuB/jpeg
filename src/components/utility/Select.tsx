...
export default function Select(props: SelectProps) {
	...
	return (
		<div onClick={toggleOpen} class="flex w-24 flex-row-reverse gap-8">
			<Show when={covered()}>
				<div class="absolute right-0 z-[100] h-full w-24 cursor-pointer"></div>
			</Show>
			<For each={props.children}>
				{(option, index) => (
					<Option
						...
						zIndex={(index() + (props.length - selected() - 1)) % props.length}
					>
						{option}
					</Option>
				)}
